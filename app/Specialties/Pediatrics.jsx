// app/pediatrics-doctors.js (or app/doctors/pediatrics.js if using static routes)
...
// Dummy Data for Pediatrics Doctors
const ALL_PEDIATRICS_DOCTORS = [
  {
    id: "ped1",
...
const PediatricsDoctorsScreen = () => {
  const insets = useSafeAreaInsets();
  const navigation = useNavigation();
  const params = useLocalSearchParams();

  const specialtyTitle = "Pediatrics";
  ...
  useEffect(() => {
    let filtered = ALL_PEDIATRICS_DOCTORS.filter(
      (doctor) =>
  ...
  const handleFilterPress = () => {
    Alert.alert(
      "Filter Options",
      "Advanced filtering for Pediatrics doctors coming soon!"
    );
  };
      ...
      >
        {displayedDoctors.length > 0 ? (
          ...
          <View style={styles.noResultsContainer}>
            {/* Using a child-related icon for pediatrics */}
            <MaterialCommunityIcons
...
export default PediatricsDoctorsScreen;
