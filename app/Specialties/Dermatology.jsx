// app/dermatology-doctors.js (or app/doctors/dermatology.js if using static routes for specialties)
...
// Dummy Data for Dermatology Doctors
// This data is specific to Dermatology, matching your image
const ALL_DERMATOLOGY_DOCTORS = [
  {
    id: "d6",
    name: "Dr. Hannah Lewis, M.D.",
    specialization: "Dermatopathology",
    profileImage:
      "https://images.unsplash.com/photo-1559839734-2b716b178d9a?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w1NzI0NTZ8MHwxfHNlYXJjaHwxNXx8ZG9jdG9yfGVufDB8fHx8MTcwNjU1NjY4N3ww&ixlib=rb-4.0.3&q=80&w=400", // Reusing dummy images for consistency
    rating: 4.9,
...
const DermatologyDoctorsScreen = () => {
  ...
  const specialtyTitle = "Dermatology";
  ...
  useEffect(() => {
    let filtered = ALL_DERMATOLOGY_DOCTORS.filter(
      (doctor) =>
    ...
    // Add more complex sorting (e.g., by rating, experience) here if needed
  ...
  const handleDoctorInfoPress = (doctor) => {
    ...
    // In a real app, navigate to a detailed doctor profile screen
    // navigation.navigate('DoctorDetail', { doctorId: doctor.id, specialty: specialtyTitle });
  };

  const handleSchedulePress = (doctor) => {
    Alert.alert("Schedule Appointment", `Book appointment with ${doctor.name}`);
    // In a real app, navigate to an appointment booking screen
  ...
  const handleQuestionPress = (doctor) => {
    ...
    // In a real app, navigate to a chat screen
  ...
  const handleFavoritePress = (doctor) => {
    ...
    // Implement favorite/unfavorite logic, perhaps updating local state or an API
  };

  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      {/* Custom Header (Same as Cardiology, but with "Dermatology" title) */}
...
const styles = StyleSheet.create({
  ...
  headerRightPlaceholder: {
    width: 28 + 10, // Match back button size + padding
  },
  subtitle: {
    fontSize: 16,
    color: COLORS.textSecondary,
...
export default DermatologyDoctorsScreen;
